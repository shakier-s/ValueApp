...
private struct EditDealView: View {
    ...
    var body: some View {
        NavigationStack { Form {
            ...
            Section("Availability") { Picker("Category", selection: $deal.category) { ForEach(["Restaurants", "Groceries", "Cafés", "Health", "Retail"], id: \.self) { Text($0) } }; DatePicker("Expires", selection: $deal.expiry, in: Date.now..., displayedComponents: .date); Stepper("\(deal.quantity) vouchers", value: $deal.quantity, in: max(1, deal.redeemed)...10_000); Button("Update shop to current location", systemImage: "mappin.and.ellipse") { proximity.enableLocation(); if let coordinate = proximity.location?.coordinate { deal.latitude = coordinate.latitude; deal.longitude = coordinate.longitude } } }
...
struct CreateDealView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Availability") { Picker("Category", selection: $category) { ForEach(["Restaurants", "Groceries", "Cafés", "Health", "Retail"], id: \.self) { Text($0) } }; DatePicker("Expires", selection: $expiry, in: Date.now..., displayedComponents: .date); Stepper("\(quantity) vouchers", value: $quantity, in: 1...10_000); Button(proximity.location == nil ? "Use current shop location" : "Current shop location added", systemImage: "mappin.and.ellipse") { proximity.enableLocation() } }
